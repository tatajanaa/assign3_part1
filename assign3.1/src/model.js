...
const model = (historicalData, forecastData) => {
  const forPeriod = (_from, _to) => {
    if (_from && _to) {
      return model(
        historicalData.filter((x) => x.time >= _from && x.time <= _to),
  ...
  const getAverageWind = function () {
    try {
      ...
      let avg = Math.round(
        array.reduce((a, b) => a + b.value, 0) / array.length
      );
  ...
  const getAverageCloud = function () {
    try {
      let array = historicalData.filter(
        (item) => item.type === "cloud coverage"
      );
  ...
  const getDominantWindDirection = function () {
    try {
      let dominant = historicalData
        .filter((item) => item.type === "wind speed")
        .map((item) => item.direction)
        .reduce(reducer, {});

      let h = Object.keys(dominant).find(
        (key) => dominant[key] === Math.max(...Object.values(dominant))
      );
